...
function renderizarCursos(state) {
  ...
  courses
    .filter((curso) => curso.state === state)
    .forEach((curso) => {
      ...
      cursoElement.innerHTML = `
            ...
            <div class="rating">
                ${"★".repeat(Math.floor(curso.score))}${"☆".repeat(
        5 - Math.floor(curso.score)
      )}
            </div>
      ...
      enterCourseBtn.addEventListener("click", (e) => {
        ...
        tempElement.innerHTML = curso.title;
        let plainText = tempElement.textContent;
        updateCurrentModeDisplay(plainText);
        showSection(4);
      });
    });
}
